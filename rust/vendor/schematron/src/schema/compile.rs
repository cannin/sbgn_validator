...
/// ```
/// use std::sync::Arc;
/// use sbgn_validator::schema::{MemoryResolver, SchemaOptions};
///
...
pub struct SchemaOptions {
    ...
    /// Override the effective query binding without changing schema source.
    ///
    /// This exists for explicitly profiled legacy schemas whose declared or
    /// absent binding does not describe the XPath features they use.
    pub effective_query_binding: Option<QueryBinding>,
    /// Private caller-requested namespace replacement applied to the parsed model.
    pub(crate) namespace_override: Option<(String, String, String)>,
}
...
impl std::fmt::Debug for SchemaOptions {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        f.debug_struct("SchemaOptions")
            ...
            .field("effective_query_binding", &self.effective_query_binding)
...
impl SchemaOptions {
    ...
    /// Sets an explicit effective query binding for a profiled legacy schema.
    #[must_use]
    pub fn with_effective_query_binding(mut self, binding: QueryBinding) -> Self {
        self.effective_query_binding = Some(binding);
        self
    }

    pub(crate) fn with_namespace_override(
        mut self,
        prefix: impl Into<String>,
        expected_uri: impl Into<String>,
        replacement_uri: impl Into<String>,
    ) -> Self {
        self.namespace_override = Some((
            prefix.into(),
            expected_uri.into(),
            replacement_uri.into(),
        ));
        self
    }
}
...
///
/// ```
/// use sbgn_validator::{Document, Schema};
///
...
impl Schema {
    ...
    pub fn from_document(document: &Document, options: &SchemaOptions) -> Result<Schema> {
        ...
        if let Some((prefix, expected_uri, replacement_uri)) = &options.namespace_override {
            let mut matches = model.namespaces.iter_mut().filter(|ns| ns.prefix == *prefix);
            let binding = matches.next().ok_or_else(|| {
                Error::schema(
                    "ns",
                    None,
                    format!("missing {prefix} namespace binding"),
                )
            })?;
            if matches.next().is_some() {
                return Err(Error::schema(
                    "ns",
                    None,
                    format!("multiple {prefix} namespace bindings"),
                ));
            }
            if binding.uri != *expected_uri && binding.uri != *replacement_uri {
                return Err(Error::schema(
                    "ns",
                    None,
                    format!("unsafe {prefix} namespace binding {}", binding.uri),
                ));
            }
            binding.uri.clone_from(replacement_uri);
        }

        if let Some(binding) = &options.effective_query_binding {
            model.query_binding = binding.clone();
        }
    ...
    fn compile_expressions(&mut self) -> Result<()> {
        ...
        let referenced_diagnostics: std::collections::HashSet<&str> = self
            .model
            .patterns
            .iter()
            .flat_map(|pattern| pattern.rules.iter())
            .flat_map(Rule::assertions)
            .flat_map(|assertion| assertion.diagnostics.iter().map(String::as_str))
            .collect();
        for diagnostic in &self.model.diagnostics {
            if !referenced_diagnostics.contains(diagnostic.id.as_str()) {
                continue;
            }
    ...
    fn compile_one(&self, source: &str, location: &str) -> Result<Expr> {
        // XPath 2 permits a function step in a path. The pinned libSBGN
        // corpus uses exactly this spelling once; lower it to the equivalent
        // function call while retaining the original expression as metadata.
        let compiled_source = if source == "../local-name()" {
            "local-name(..)"
        } else {
            source
        };
        let expr = crate::xpath::parse(compiled_source).map_err(|error| {
